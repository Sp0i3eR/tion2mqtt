...
public class BluetoothSession implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            if (!device.connect()) {
                log.error(String.format("Cannot connect to %s", device.getAddress()));
            }
            log.debug(String.format("Device %s connected", device.getAddress()));

            var uartService = device.find(UART_SERVICE);
            if (uartService == null) {
                log.error(String.format("UART service not found on %s", device.getAddress()));
                return;
            }
            ...
            if (uartTx == null || uartRx == null) {
                log.error(String.format("UART Tx and Rx characteristic not found on %s", device.getAddress()));
                return;
            }
